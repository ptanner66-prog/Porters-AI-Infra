...
- [ ] **F3 (v0.2 polish) — Tighten Karpathy §8.3 enforcement mapping** — In root CLAUDE.md §8.3 (Surgical Changes), the current mapping lists the `adverse` integration-wiring check. `adverse` is about completeness coverage, which is a looser fit than the principle requires. Replace with: code-reviewer sub-agent's blast-radius + scope-drift checks, chen sub-agent's "do not propose rewrites when precise fixes are available" directive, and the `100` rigor skill's "no fix without proven root cause" rule.
